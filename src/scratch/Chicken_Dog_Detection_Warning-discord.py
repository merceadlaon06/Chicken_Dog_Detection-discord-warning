...
VIDEO_PATH = r"C:\Users\dell\Downloads\Merce Downloads- 2_15_2026\Video_Generation_With_Dog_And_Chickens.mp4"  # input video
...
DISCORD_COOLDOWN_SEC = 30   # re-notify at most every N seconds while dog stays present
SEND_FRAME_IMAGE = True     # ✅ updated to True (attach current frame image)
...
dog_ids = [cid for cid, name in names.items() if name.lower() == DOG_CLASS_NAME.lower()]
if not dog_ids:
...
# DISCORD HELPER
# ----------------------------
def send_discord_alert(message: str, frame=None) -> None:
    """
    Sends a Discord notification via webhook.
    If SEND_FRAME_IMAGE is True and frame is provided, attaches a JPEG snapshot.
    """
    if not DISCORD_WEBHOOK_URL or DISCORD_WEBHOOK_URL.startswith("PASTE_"):
        print("[WARN] Discord webhook URL not set. Skipping Discord notifications.")
        return

    try:
        if SEND_FRAME_IMAGE and frame is not None:
            ok, buf = cv2.imencode(".jpg", frame)
            if not ok:
                print("[WARN] Failed to encode frame for Discord image.")
                return
            files = {"file": ("alert.jpg", buf.tobytes(), "image/jpeg")}
            data = {"content": message}
            requests.post(DISCORD_WEBHOOK_URL, data=data, files=files, timeout=8)
        else:
...
while True:
    ...
    # Time reference for blink + cooldown
    now = time.time()

    # ----------------------------
    # DISCORD NOTIFY (edge-trigger + cooldown)
    # ----------------------------
    if dog_present:
        if (not was_dog_present) or (now - last_discord_sent >= DISCORD_COOLDOWN_SEC):
            ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            # Include best confidence (highest among boxes) for nicer messages
            best_conf = max([b[4] for b in dog_boxes]) if dog_boxes else 0.0
            send_discord_alert(
                f"🚨 WARNING: Dog detected! (conf={best_conf:.2f}) at {ts}",
                frame=frame
            )
            last_discord_sent = now

    was_dog_present = dog_present
